...
/**
 * Filter + list content for viewing application logs. Rendered directly (no
 * isOpen/onClose) so it can be reused both standalone (LogsTab) and inside a
 * Modal wrapper if ever needed again.
 */
export const LogViewerPanel: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div className="flex-1 overflow-auto border border-slate-200 dark:border-slate-700 rounded-xl divide-y divide-slate-100 dark:divide-slate-800 font-mono text-[11px] min-h-0">
      ...
      {logDir && <p className="text-[11px] text-slate-400 shrink-0">{tf(t.logs.location, { path: logDir })}</p>}
    </div>
  );
};
